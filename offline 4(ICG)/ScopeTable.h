...
void SetId(){
   ...
   static unsigned int SDBMHash(string str) {
    ...
    bool Insert(string name, string type, string globalName)
    {
    ...
    {
       // cout<<"vitorer insert e dhukse"<<endl;
        SymbolInfo* node= LookUp(symbol->getName());
        //ei line e atkaise
        if(node!=nullptr)
        {
            //cout << "< " << name << "," << type <<" > already exist in the currentScopeTable"<<endl;
        ...
        else
        {
            // cout<<"vitorer else e dhukse"<<endl;
            SymbolInfo* temp = new SymbolInfo ;
            int index= SDBMHash(symbol->getName()) % numOfBuckets;
            SymbolInfo * cur= ArrayOfSymbolTable[index];
            temp=symbol;
            // cout<<"Else kaz kore"<<endl;
    ...
    void print(ofstream& logout)
    {
       ...
       for ( i = 0; i < numOfBuckets; i++)
       {
           ...
           {
               logout<<" < " <<listPrint->getName()<< " , "<<listPrint->getType()<<" >";
        ...
        for (int i = 0; i < numOfBuckets; i++)
        {
           ...
           // SymbolInfo* temp = new SymbolInfo();
            ...
            while (current!=nullptr) {
                temp= current->getSymbolNextInChain();
              //  current=current->getSymbolNextInChain();
              //  delete temp;
              delete current;
              current= temp;
